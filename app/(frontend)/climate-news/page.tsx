...
import { getClimateNews } from "@/lib/getNewsByCategory";
...
export default function ClimateNewsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      const climateNews = await getClimateNews();
      console.log("climate news:", climateNews);
      setNews(climateNews);
      setLoading(false);
    }
    fetchData();
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
          ...
          <div className="flex flex-col justify-center">
            ...
            <p className="text-gray-700 text-base leading-relaxed">
              {news[0].summary || "Latest climate news and environmental updates."}
            </p>
            <a href={`/climate-news/${news[0].slug}`} className="mt-6 border-2 border-black px-8 py-3 font-bold text-sm tracking-widest hover:bg-black hover:text-white transition inline-block">
              READ MORE
            </a>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {paginatedNews.map((item: any, idx: number) => (
              ...
              >
                <a href={`/climate-news/${item.slug}`} className="block">
